...
void KDtreeFlann::setInputCloud(pointCloud _input)
{
	try
	{
		...
		flann_index_ = new FLANNIndex ( ::flann::Matrix<double> ((&input[0]), original_points, dim), ::flann::KDTreeSingleIndexParams (15)); // max 15 points/leaf
	...
	std::vector<double> &pointKSquaredDistance)
{
	...
	::flann::Matrix<double> distances_matrix(&pointKSquaredDistance[0], 1, _k);

	return(	flann_index_->knnSearch (::flann::Matrix<double> (&queryPt[0], 1, dim), 
		indices_matrix, distances_matrix,
		_k, searchParam_));
}

// Provides number of points in given radius
int KDtreeFlann::radiusSearch(
	Point _point,
	double _radius,
	...
	std::vector<double> &dists_radius)
{
	...
	int return_value = (flann_index_->radiusSearch (::flann::Matrix<double> (&queryPt[0], 1, dim), 
		indices, dists,
