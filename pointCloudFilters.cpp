...
void Filter3D::statisticalOutlierRemoval(
	int _k, 
	pointCloud input_, 
	pointCloud &output)
{
	...
	indices.resize(size);
	size_t oii = 0, rii = 0;  // oii = output indices iterator, rii = removed indices iterator

	// First pass: Compute the mean distances for all points with respect to their k nearest neighbours
	...
	sq_sum = std::accumulate(distances.begin(), distances.end(), 0, [](double &lhs, double &rhs){return (lhs + (rhs*rhs)) ;});
	...
	for (size_t iii = 0; iii < static_cast<int> (input_.size()); ++iii)  // iii = input indices iterator
	{
	...
	pointCloud &output)
{
	...
	{
		int ijk0 = static_cast<int> (floor(input_[it].x / _Delta) - static_cast<double> (min_b_[0]));
		int ijk1 = static_cast<int> (floor(input_[it].y / _Delta) - static_cast<double> (min_b_[1]));
		int ijk2 = static_cast<int> (floor(input_[it].z / _Delta) - static_cast<double> (min_b_[2]));

		// Compute the centroid leaf index
		int idx = ijk0 * divb_mul_[0] + ijk1 * divb_mul_[1] + ijk2 * divb_mul_[2];
		index_vector.push_back(cloud_point_index_idx(static_cast<size_t> (idx), it));
	}
	auto compare = [](const cloud_point_index_idx& lhs, const cloud_point_index_idx& rhs) {return (lhs.idx == rhs.idx); };

	auto predict = [](const cloud_point_index_idx& lhs, const cloud_point_index_idx& rhs) {return lhs.idx < rhs.idx; };
	...
	while (index < index_vector.size())
	{
		...
		{
			++total;
			first_and_last_indices_vector.push_back(std::pair<size_t, size_t>(index, i));
		}
		index = i;
	}
	output.resize(total);
...
void Filter3D::MakeSmoothPointCloud(
	size_t _k, 
	...
	pointCloud &output)
{
	...
	for (size_t i = 0; i < size; i++)
	{
		if (tree.radiusSearch(input_[i], radius, indices_radius, dists_radius) > 0)
		{
	...
	std::vector<pointCloud>& output)
{
	...
	for (size_t i = 0; i < input_.size(); i++)
	{
		...
		{
			if (!tree.radiusSearch(input_[clusterPoints[index]], radius, raidusIndices, raidusDistances))
			{
				index++;
				continue;
			}
		...
		if (clusterPoints.size() >= minClusterSize && clusterPoints.size() <= maxClusterSize)
		{
			...
			sortedCloud.erase(std::unique(sortedCloud.begin(), sortedCloud.end()), sortedCloud.end());
	...
	sort(output.begin(), output.end(), [](pointCloud &lhs, pointCloud &rhs) { return lhs.size() > rhs.size(); });
	output.shrink_to_fit();
}
	...
	std::vector<std::vector<int>>& output)
{
	...
	sort(output.begin(), output.end(), [](std::vector<int> &lhs, std::vector<int> &rhs) { return lhs.size() > rhs.size(); });
	output.shrink_to_fit();
}

void Filter3D::findCentroidAndCovarianceMatrix(pointCloud cloud,
	...
	std::vector<normalsAndCurvature> &output)
{
	...
	{
		tree.knnSearch(cloud[i], numberOfneighbour, pointIdxKSearch, pointKSquaredDistance);
		...
		findCentroidAndCovarianceMatrix(processingCloud, covariance_matrix, centroid);
		...
		std::vector<double>::iterator result = std::min_element(std::begin(eval), std::end(eval));
		...
		if (((output[i].nx * cloud[i].x) + (output[i].ny * cloud[i].y) + (output[i].nz * cloud[i].z)) > 0)
		{
			output[i].nx *= -1;
			output[i].ny *= -1;
			output[i].nz *= -1;
		...
		double eig_sum = sacledCVM.coeff(0) + sacledCVM.coeff(4) + sacledCVM.coeff(8);
		if (eig_sum != 0)
			output[i].curvature = fabsf(es.eigenvalues()[minEvalIndex].real() / eig_sum);
		else
...
void Filter3D::PassthroughFilter(
	pointCloud& input,
	pointCloud& output)
{
	...
	if (x_passthough_limits_set && y_passthough_limits_set && z_passthough_limits_set)
	{
		it = std::copy_if(points.begin(), points.end(), output.begin()
			, [this](const pointCloud &p) {
			return  (InRange(p.x, x_limits.first, x_limits.second) && InRange(p.y, y_limits.first, y_limits.second) && InRange(p.z, z_limits.first, z_limits.second));
		});
	}
	else if (x_passthough_limits_set && y_passthough_limits_set)
	{
		it = std::copy_if(points.begin(), points.end(), output.begin()
			, [this](const pointCloud &p) {
			return  (InRange(p.x, x_limits.first, x_limits.second) && InRange(p.y, y_limits.first, y_limits.second));
		});
	}
	else if (x_passthough_limits_set && z_passthough_limits_set)
	{
		it = std::copy_if(points.begin(), points.end(), output.begin()
			, [this](const pointCloud &p) {
			return  (InRange(p.x, x_limits.first, x_limits.second) && InRange(p.z, z_limits.first, z_limits.second));
		});
	}
	else if (y_passthough_limits_set && z_passthough_limits_set)
	{
		it = std::copy_if(points.begin(), points.end(), output.begin()
			, [this](const pointCloud &p) {
			return  (InRange(p.y, y_limits.first, y_limits.second) && InRange(p.z, z_limits.first, z_limits.second));
		});
	}
